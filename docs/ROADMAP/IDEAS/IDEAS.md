...
Speculative feature designs and open questions with no active decision to
build them — moved out of `docs/ROADMAP.md` so that file stays a short,
current "what's actually open" tracker. Nothing here is scheduled; a design
sketch here is a starting point if/when someone picks it up, not a
commitment. [`docs/FEATURES.md`](../../FEATURES.md) is the authoritative list
of what's actually shipped — if a module/command named below doesn't appear
there, it doesn't exist yet.

## Table of Contents

- [Ad-hoc notes](#ad-hoc-notes)
...
If any of these gets scoped for real, the natural next-pick order (from a
pass through this list): **Notification Thresholds** first — additive, no
dashboard changes, the rule schema below is already concrete enough to
implement directly. **Repository Groups/Tags** next — config plus a thin
aggregation layer over existing `analytics.query_all_repos`. **Interactive
Chart Navigation** is the largest of the three (new stateful UI component,
its own keymaps). **Webhook Integration** stays last regardless — it needs an
HTTP server in pure Lua, cross-platform NAT/ngrok handling, and HMAC
verification (4-5 weeks per its own estimate below), by far the largest
effort and the least aligned with the plugin's current "local polling" model.
