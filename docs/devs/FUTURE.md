...
- [Medium Features (v1.4.0 - v1.6.0)](#medium-features-v140---v160)
  ...
  - [Custom Date Range Presets](#custom-date-range-presets)
...
Predefined and user-defined date range presets for quick access to common time periods.

**Goals:**
- Quick access to common ranges (last week, last month, last quarter)
  ...
  date_presets = {
    ...
    custom = {
      ...
      fiscal_year = function()
        ...
        local fy_year = now.month >= fy_start_month
          and now.year
          or now.year - 1
      ...
      current_sprint = function()
        -- Calculate current 2-week sprint
        local now = os.time()
        local sprint_length = 14 * 86400  -- 14 days in seconds

        -- Assuming sprints start on Monday
        local monday = now - (os.date("*t", now).wday - 2) * 86400
        local sprint_start = monday - (monday % sprint_length)

        local start_date = os.date("%Y-%m-%d", sprint_start)
        local end_date = os.date("%Y-%m-%d", sprint_start + sprint_length - 86400)
...
**Usage:**
```vim
" Using preset names directly
:GithubStatsShow username/repo clones last_month
:GithubStatsChart username/repo views this_quarter
:GithubStatsDiff username/repo clones last_month this_month

" Autocomplete shows:
:GithubStatsShow username/repo clones <Tab>
" Suggestions: today, yesterday, last_week, last_month, ...
```

**Preset Resolver:**
```lua
-- lua/github_stats/date_presets.lua
local M = {}
...
function M.resolve(preset_name)
  local builtins = {
    ...
    last_week = function()
      ...
      return os.date("%Y-%m-%d", week_ago), os.date("%Y-%m-%d", now)
    end,

    -- ... more builtins
  }

  local custom = require("github_stats").config.get().date_presets.custom or {}

  local resolver = builtins[preset_name] or custom[preset_name]
  if not resolver then
    return nil, string.format("Unknown preset: %s", preset_name)
  end
...
```lua
-- In usercommands/show.lua
function M.complete(arg_lead, cmd_line, cursor_pos)
  -- ... existing logic

  -- If completing date parameter, show presets
...
**File:** `lua/github_stats/date_presets.lua`
