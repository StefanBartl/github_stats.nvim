...
function M.execute(_args)
  ...
  if not cfg then
    config.notify("[github-stats] Config not loaded", "error")
    return
  end
  ...
  local lines = {
    ...
    str_format("Background fetch: %s", (cfg.background == nil or cfg.background.enabled ~= false) and "enabled" or "disabled"),
  }
  ...
  if fetcher.last_fetch_summary then
    ...
    if not summary then
      config.notify("[github-stats] summary is nil in debug usercommand", "warn")
      return
    end
