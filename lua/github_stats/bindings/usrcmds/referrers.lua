---@module 'github_stats.bindings.usrcmds.referrers'
...
function M.execute(args)
  ...
  if #parts < 1 then
    config.notify("[github-stats] Usage: :GithubStats referrers {repo} [limit]", "error")
    return
  end
  ...
  if #referrers == 0 then
    config.notify("[github-stats] No referrer data available", "info")
    return
  end
  ...
  for i, ref in ipairs(referrers) do
    ...
    table.insert(
      lines,
      str_format("    Count: %s, Uniques: %s", utils.format_number(ref.count), utils.format_number(ref.uniques))
    )
  end
